...
export class StockReceiptRepository implements IStockReceiptRepository {
  async saveHeader(header: StockReceiptHeader): Promise<void> {
    await sql`
  ...
  async saveLine(line: StockReceiptLine): Promise<void> {
    await sql`
  ...
  async findHeaderById(id: string): Promise<StockReceiptHeader | null> {
  ...
  async findLinesByReceiptId(headerId: string): Promise<StockReceiptLine[]> {
    const result = await sql`
